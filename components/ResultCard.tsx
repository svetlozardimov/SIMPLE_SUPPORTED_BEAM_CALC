...
const ResultCard: React.FC<ResultCardProps> = ({ title, value, unit, onClick, isExpanded, validationStatus = 'none', limitValue = null, combinationDescription }) => {
  ...
  const baseClasses = 'relative bg-slate-700/50 rounded-lg p-4 text-center cursor-pointer transition-all duration-300 result-card flex flex-col justify-between';
  ...
  return (
    ...
    >
      <div>
        ...
        <div className="flex items-center justify-center gap-2 h-8 my-1">
         ...
         {limitValue !== null && validationStatus !== 'none' && (
            <p className={`text-xs ${validationStatus === 'pass' ? 'text-green-400' : 'text-red-400'}`}>
                Допустимо: {(limitValue * 1000).toFixed(2)} mm
            </p>
        )}
      </div>
