...
const ResultCard: React.FC<ResultCardProps> = ({ title, value, unit, onClick, isExpanded, validationStatus = 'none', limitValue = null }) => {
  const formattedValue = value !== null ? value.toLocaleString('bg-BG', { minimumFractionDigits: 3, maximumFractionDigits: 3 }) : '---';

  const baseClasses = 'relative bg-slate-700/50 rounded-lg p-4 text-center cursor-pointer transition-all duration-300 result-card';
    ...
    >
      ...
      <div className="flex items-center justify-center gap-2 h-8">
          ...
          <p className="text-2xl font-bold text-cyan-400">
            {formattedValue} <span className="text-lg text-slate-300">{unit}</span>
          </p>
      </div>
       {limitValue !== null && validationStatus !== 'none' && (
          <p className={`text-xs mt-1 ${validationStatus === 'pass' ? 'text-green-400' : 'text-red-400'}`}>
              Допустимо: {(limitValue * 1000).toFixed(3)} mm
          </p>
      )}
    </div>
  );
