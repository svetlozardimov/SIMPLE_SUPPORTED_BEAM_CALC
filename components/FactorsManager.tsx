...
const FactorInput: React.FC<{
    ...
    <div>
        ...
        <div className="relative">
            <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-cyan-400 pointer-events-none font-semibold text-sm">{symbol}</span>
            <input
                type="number"
                value={value}
                onChange={onChange}
                step="0.05"
                className="w-full bg-slate-700 border-2 border-slate-600 rounded-md py-1.5 pl-8 pr-3 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition text-sm"
            />
        </div>
    </div>
);
        ...
        <div className="bg-slate-900/50 rounded-lg overflow-hidden">
            ...
            >
                <div className="p-3 border-t border-slate-700 space-y-4">
                    <div>
                        <h3 className="font-semibold text-cyan-300 mb-2 text-sm">Коефициенти за сигурност (ULS) - γ</h3>
                     ...
                     <div>
                        <h3 className="font-semibold text-cyan-300 mb-2 text-sm">Комбинационни коефициенти (SLS) - ψ₀</h3>
                         <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
